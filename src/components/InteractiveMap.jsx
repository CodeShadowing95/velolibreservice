    ...
    <div className="relative w-screen h-screen overflow-hidden z-[1px]">
      ...
      >
        ...
        {Object.keys(stationFromContract).length !== 0 ?
          <Marker longitude={stationFromContract.position.longitude} latitude={stationFromContract.position.latitude} style={{ zIndex: 1 }} pitchAlignment="viewport">
            <div className="flex relative justify-center cursor-pointer hover:scale-125 transition-transform">
              <img src="/public/map-marker.png" alt="map marker" className="w-8 h-8 animate-bounce" />
            </div>
          </Marker>
          :
          // Sinon on affiche tous les markers
          stations.map((station) => (
            <Marker longitude={station.position.longitude} latitude={station.position.latitude} key={station.name} style={{ zIndex: 1 }} pitchAlignment="viewport" onClick={() => handleLocationChange(station.position.latitude, station.position.longitude, station.number)}>
