          ...
          {stations.length ?
            stations.map((station) => (
              ...
              <div key={station.number} className={`flex flex-col items-center cursor-pointer transition-transform bg-white min-w-60 rounded-xl bg-clip-padding backdrop-filter backdrop-blur-md bg-opacity-50 shadow-xl ${station.number === stationIndex ? "border-[5px] border-blue-500/50 scale-100" : "border hover:scale-95"}`} onClick={() => goToPosition(station?.position.latitude, station?.position.longitude, station.number)}>
                ...
                <div className="w-full flex flex-wrap items-end gap-1 mt-1 px-2">
                  {/* status */}
                ...
                <div className="flex flex-col gap-2 w-full px-2">
                  <div className={`flex items-center gap-1 rounded-full px-2 py-[2px]`}>
                    ...
                    <p className="text-xs font-medium">Capacité totale de la station: <span className="text-amber-600 font-extrabold">{station?.totalStands.capacity > station?.mainStands.capacity ? station?.totalStands.capacity : station?.mainStands.capacity}</span></p>
                  </div>
                  <div className="flex flex-wrap items-center w-full px-2 gap-2 mt-1">
